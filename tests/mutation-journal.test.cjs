...
test('applied mutation is rolled back safely after restart', t => {
  ...
  const id = journal.begin({
    sessionId: 's1', step: 1, operation: 'modify',
    destinationPath: target, originalContent: 'before\n',
  });
  ...
  assert.equal(journal.getById(id).state, 'ROLLED_BACK');
  db.close();
});
...
test('recovery refuses to overwrite a later developer edit', t => {
  ...
  const id = journal.begin({
    sessionId: 's2', step: 2, operation: 'modify',
    destinationPath: target, originalContent: 'before\n',
  });
  ...
  assert.equal(fs.readFileSync(target, 'utf8'), 'developer\n');
  assert.equal(journal.getById(id).state, 'DIVERGED');
});
...
test('successful mutation transaction reaches committed journal state', async t => {
  ...
  const result = await tx.execute(3, {
    tool: 'write_file',
    args: { path: 'created.txt', content: 'durable\n' },
  });
