...
function setup(t) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'cos-indexed-retrieval-'));
...
test('large vector corpus uses indexed LSH candidates instead of loading every sketch', async t => {
  ...
  assert.ok(db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='embedding_lsh_buckets'").get());
  assert.ok(Number(db.prepare('SELECT COUNT(*) AS count FROM embedding_lsh_buckets').get().count) >= chunks.length);

  const originalPrepare = db.prepare.bind(db);
  db.prepare = function(sql) {
    if (/SELECT\s+id\s*,\s*sketchBlob\s+FROM\s+embeddings_cache/i.test(String(sql))) throw new Error('full sketch scan forbidden for large corpus');
...
test('legacy persisted vectors are deterministically backfilled into the candidate index', t => {
  ...
  db.prepare(`INSERT INTO embeddings_cache
    ...
    VALUES (?,?,?,?,?,?,?,?)`).run('legacy', path.join(dir, 'legacy.ts'), 'legacy-hash', 'legacy target', blob, blob, vector.length, Date.now());
